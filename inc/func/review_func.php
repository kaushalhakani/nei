...
{
	$sql = "SELECT first_name, last_name FROM tbl_member WHERE member_id =".$author_id; 
...
{
	print "<span class=\"pageHeadline\"><h2>$title</h2></span>";
}
...
function PrintThreadList($page=0)
{
	...
	{
		// Printing results in HTML
		print "<div><table border=\"0\" cellpadding=\"2\" cellspacing=\"2\">";
		$count = 0;
		while ($count < $g_TopicsPerPage and $line = mysql_fetch_row($result)) 
		{
			$author_name = get_author_name($line[2]);
			$to_print = "<tr><td width=\"930\"><a href=\"$g_ThisPage?cmd=show&amp;thread=$line[0]";
			if( $g_DisablePostsInURLs != "1" )
				$to_print .= "&amp;posts=$line[4]";
			$to_print .= "\"><big><b>$line[1]</b></big></a> ";
			print "$to_print";
			
			$to_print = "<tdalign=\"right\">&nbsp;</td><td><a href=\"$g_ThisPage?cmd=deleteThread&amp;thread=$line[0]";
			if( $g_DisablePostsInURLs != "1" )
				$to_print .= "&amp;posts=$line[4]";
			$to_print .= "\">";
			...
			print "<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$line[8]."</tr></td>";
			$temp1 = explode("/",$line[7]);
			print "<tr><td><i>Uploaded File :</i>  <b><a href=\"../review/upload/".$temp1[1]."\">". $temp1[1]."</a></b></tr></td>";
			print "<tr><td><i>".$author_name['first_name']." ".$author_name['last_name'];
	...
	else
	{
		...
		echo "<form method=\"get\" name=\"author_select\" id=\"author_select\" action=\"\">
				View Posts by : <select name=\"author\" onchange=\"thread_submit()\">";
...
function PrintThreadName($thread)
{
	...
	$query = "SELECT Title, abstract FROM $g_ThreadListTableName WHERE ThreadID = $thread";
	$result = mysql_query($query)
...
{
	global $g_ThisPage, $g_MessageListTableName, $g_ThreadListTableName;
	...
	$thread_query = "SELECT *,DATE_FORMAT(CreationDate, '%W, %M %e, %Y') AS mydatestring1 FROM $g_ThreadListTableName WHERE ThreadID = $thread";
	$thread_result = mysql_query($thread_query)
	    or DBError("Failed while trying to find the thread in the database");
	$thread_row = mysql_fetch_row($thread_result);
	
	// print out a heading
	PageHeader();
	PrintTitle($thread_row[1]);
		
	print "<div>$thread_row[8]</div><br />";
	...
	{	
		...
		if ($line[3] == "")
			...
			print "<a title=\"Click to send private email\" href=\"$g_ThisPage?cmd=newmailform&amp;message=$line[0]\">".$author_name['first_name']." ".$author_name['last_name']."</a>";
...
function CreateNewThread($title, $author_id, $email, $file_path, $msg)
{
	global $g_ThreadListTableName;
	...
	$query = "INSERT INTO $g_ThreadListTableName VALUES (NULL, '$title', '$author_id', '$email', '$messagecount', '$datetime', '$datetime', '$file_path', '$msg')";
	echo $query;
	$result = mysql_query($query)
...
function ShowNewTopicForm($pageTitle, $title, $message, $error)
{
	...
	if( strlen($error) > 0 )
		PrintError($error);	
	?>
	<form method="post" action="<?php print $g_ThisPage; ?>" enctype="multipart/form-data">
	  <div align="left">
	    <table border="0" cellpadding="0" cellspacing="0" width="500">
          ...
          <td valign="top" align="left">Upload<span class="requiredWarning"> * </span> <b>:</b> </td>
	        <td valign="top" align="left"><input class="formBox" type="file" name="file" id="file" size="51" value="" /></td>
	      </tr>
	      <tr>
	        <td valign="top" align="left">Abstract<span class="requiredWarning"> * </span> <b>:</b> </td>
