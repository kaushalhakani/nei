...
require_once('../inc/func/review_func.php');
...
$g_Title = "NEI - Review Forum";	// The title of this discussion forum
...
$g_ThisPage = "review.php";			// the name of this page
$g_Password = "";			// to run database script
...
$g_MessageListTableName = "tbl_review_comment";	// the message list table name in the database
$g_ThreadListTableName = "tbl_review";		// the thread list table name in the database
...
else if ($cmd == "submitnew")
{
	...
	{
		ShowNewTopicForm("Start a New Topic", $title, $msg, "Return Code: " . $_FILES["file"]["error"] . "<br />");
	}
	else
	{
		
		if (/*(($_FILES["file"]["type"] == "image/gif") || ($_FILES["file"]["type"] == "image/jpeg") || ($_FILES["file"]["type"] == "image/pjpeg")) && ($_FILES["file"]["size"] < 20000)*/ true)
		{
		...
		if ($title != "" and $msg != "")
		{
			$thread = CreateNewThread($title, $_SESSION['ID'], $email, "upload/" . $_FILES["file"]["name"], $msg);
		}
